...
pub mod v1_2_0 {
	...
	pub fn migrate<T: Config>() -> Weight {
		...
		// AresPrice
		let _ = AresPrice::<T>::translate::<Vec<AresPriceData<T::AuthorityAres, T::BlockNumber>>, _,>(|maybe_old_key, maybe_old_val| {
			...
			read_count+=1;
			write_count+=1;
			let new_data = maybe_old_val.into_iter().map(
				|c| {
					AresPriceData {
						price: c.price,
		...
		OldPurchasedOrderPool::<T>::iter().map(|(k1,k2,val)| {
			// PurchasedOrderPool::<T>::swap(k1.clone(), k2.clone(), k1, crate::Pallet::<T>::get_stash_id_or_default(&k2));
			log::info!(
				target: "runtime::ares-oracle",
		...
		// PurchasedPricePool
		let _ = PurchasedPricePool::<T>::translate::<Vec<AresPriceData<T::AuthorityAres, T::BlockNumber>>, _,>(|_key1, _key2_, maybe_old_val| {
			log::info!(
				target: "runtime::ares-oracle",
		...
		// AresAbnormalPrice
		let _ = AresAbnormalPrice::<T>::translate::<Vec<AresPriceData<T::AuthorityAres, T::BlockNumber>>, _,>(|maybe_old_key, maybe_old_val| {
			log::info!(
				target: "runtime::ares-oracle",
			...
			let new_data = maybe_old_val.into_iter().map(
				|c| {
					...
					(new_data, AvgPriceData{ integer: 0, fraction_len: 0 })
				}
			).collect::<Vec<_>>();
		...
		// HttpErrTraceLogV1
		let _ = <HttpErrTraceLogV1<T>>::translate::<Vec<HttpErrTraceData<T::BlockNumber, T::AuthorityAres>>, _>(|maybe_old_trace_log| {
			read_count+=1;
			write_count+=1;
