...
decl_module! {
    pub struct Module<T: Config<I>, I: Instance=DefaultInstance> for enum Call where origin: <T as frame_system::Config>::Origin {
        ...
        fn close(origin,
            ...
            let is_ares = Self::is_ares_call(proposal.clone());
            ...
            if approved {
                ...
                if !is_ares || (is_ares && no_votes > voting.threshold) {
                    Self::deposit_event(RawEvent::Closed(proposal_hash, yes_votes, no_votes));
            ...
            } else if !is_ares || (is_ares && no_votes > voting.threshold) {
                Self::deposit_event(RawEvent::Closed(proposal_hash, yes_votes, no_votes));
                let proposal_count = Self::do_disapprove_proposal(proposal_hash);
                return Ok((
            ...
            return Err(Error::<T, I>::TooEarly.into())
        }
    ...
    fn do_disapprove_proposal(proposal_hash: T::Hash) -> u32 {
        ...
        if proposal_result.is_ok() {
            ...
            Self::check_ares_call(
                proposal,
                proposal_hash,
                |_, proposal_hash, deposit| -> DispatchResult {
                    T::ChallengeFlow::slash_proposer(proposal_hash)
                },
                || -> DispatchResult { Ok(()) },
            );
        }
...
//For Ares Call
pub struct EnsureProportionNoMoreThan<N: U32, D: U32, AccountId, I = DefaultInstance>(
    sp_std::marker::PhantomData<(N, D, AccountId, I)>,
);
impl<
        O: Into<Result<RawOrigin<AccountId, I>, O>> + From<RawOrigin<AccountId, I>>,
        ...
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    record(Event::Collective(RawEvent::Voted(
                        1,
                        hash.clone(),
                        true,
                        1,
        ...
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    record(Event::CollectiveMajority(RawEvent::Voted(
                        1,
                        hash.clone(),
                        true,
                        1,
                    ...
                    record(Event::CollectiveMajority(RawEvent::Closed(
                        hash.clone(),
                        5,
                        0
                    ))),
