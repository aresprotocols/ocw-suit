...
use frame_support::{
	...
	traits::{
		Backing, ChangeMembers, Currency, EnsureOrigin, Get, GetBacking, InitializeMembers, IsSubType, StorageVersion,
	},
	weights::{GetDispatchInfo, Weight},
};
use pallet_ares_challenge::ChallengeFlow;
...
type BalanceOf<T> =
	<<T as pallet_ares_challenge::Config>::Currency as Currency<<T as frame_system::Config>::AccountId>>::Balance;
...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config<I: 'static = ()>: frame_system::Config + pallet_ares_challenge::Config {
		...
		type ChallengeFlow: ChallengeFlow<Self::AccountId, Self::Hash, BalanceOf<Self>>;

		type AresProposalMinimumThreshold: EnsureOrigin<<Self as Config<I>>::Origin>;

		type AresProposalMaximumThreshold: EnsureOrigin<<Self as Config<I>>::Origin>;
	}
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn propose(
			...
			Self::check_ares_call(
				...
				proposal_hash,
				|dest, proposal_hash, deposit| -> DispatchResult {
		...
		pub fn close(
			...
			if approved {
				let (proposal, len) =
					Self::validate_and_get_proposal(&proposal_hash, length_bound, proposal_weight_bound)?;
		...
		pub fn disapprove_proposal(origin: OriginFor<T>, proposal_hash: T::Hash) -> DispatchResultWithPostInfo {
			ensure_root(origin)?;
			let proposal_count = Self::do_disapprove_proposal(proposal_hash);
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	fn validate_and_get_proposal(
		...
		let proposal_len = storage::read(&key, &mut [0; 0], 0).ok_or(Error::<T, I>::ProposalMissing)?;
	...
	fn do_approve_proposal(
		...
		//TODO handler return error
		Self::check_ares_call(
			proposal.clone(),
			proposal_hash,
			|_, proposal_hash, deposit| -> DispatchResult { T::ChallengeFlow::slash_data_feed(proposal_hash) },
			|| -> DispatchResult { Ok(()) },
		);
	...
	fn do_disapprove_proposal(proposal_hash: T::Hash) -> u32 {
		// disapproved
		let proposal_result: Result<<T as Config<I>>::Proposal, Error<T, I>> =
			ProposalOf::<T, I>::get(&proposal_hash).ok_or(Error::<T, I>::ProposalMissing);
		if proposal_result.is_ok() {
			let proposal: <T as Config<I>>::Proposal = proposal_result.unwrap();
			//TODO handler return error
			Self::check_ares_call(
				proposal,
				proposal_hash,
				|_, proposal_hash, deposit| -> DispatchResult { T::ChallengeFlow::slash_proposer(proposal_hash) },
				|| -> DispatchResult { Ok(()) },
			);
		}
	...
	where
		FA: FnOnce(&<T::Lookup as StaticLookup>::Source, T::Hash, BalanceOf<T>) -> DispatchResult,
...
impl<O: Into<Result<RawOrigin<AccountId, I>, O>> + From<RawOrigin<AccountId, I>>, N: U32, AccountId, I> EnsureOrigin<O>
	for EnsureMembers<N, AccountId, I>
{
...
pub struct EnsureProportionMoreThan<N: U32, D: U32, AccountId, I: 'static>(PhantomData<(N, D, AccountId, I)>);
...
pub struct EnsureProportionAtLeast<N: U32, D: U32, AccountId, I: 'static>(PhantomData<(N, D, AccountId, I)>);
impl<O: Into<Result<RawOrigin<AccountId, I>, O>> + From<RawOrigin<AccountId, I>>, N: U32, D: U32, AccountId, I>
