	...
	frame_system::Config + pallet_ares_demo::Config
{
	...
	type CheckFlow: CheckFlow<Self::AccountId, Self::Hash, BalanceOf<Self>>;
		...
		fn propose(origin,
			...
			Self::check_ares_call(
				...
				|dest, proposal_hash, deposit|->DispatchResult{
					...
					T::CheckFlow::prepare( &who, &dest, proposal_hash, deposit)
				},
				|| -> DispatchResult {
					ensure!(members.contains(&who), Error::<T, I>::NotMember);
					Ok(())
	...
	fn do_approve_proposal(
		...
		Self::check_ares_call(
			...
			|_, proposal_hash, deposit| -> DispatchResult {
				T::CheckFlow::pass(proposal_hash)
			},
			|| -> DispatchResult { Ok(()) },
		);
	...
	fn do_disapprove_proposal(proposal_hash: T::Hash) -> u32 {
		...
		if proposal_result.is_ok() {
			...
			Self::check_ares_call(
				proposal,
				proposal_hash,
				|_, proposal_hash, deposit| -> DispatchResult { T::CheckFlow::slash(proposal_hash) },
				|| -> DispatchResult { Ok(()) },
			);
		}
