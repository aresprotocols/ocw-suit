...
fn motions_all_first_vote_free_works() {
	new_test_ext().execute_with(|| {
		...
		let vote_rval: DispatchResultWithPostInfo = Collective::vote(Origin::signed(2), hash, 0, true);
		...
		let vote_rval: DispatchResultWithPostInfo = Collective::vote(Origin::signed(2), hash, 0, false);
		assert_eq!(vote_rval.unwrap().pays_fee, Pays::Yes);

		// For the motion, acc 3's first vote, expecting Ok with Pays::No.
		let vote_rval: DispatchResultWithPostInfo = Collective::vote(Origin::signed(3), hash, 0, true);
		assert_eq!(vote_rval.unwrap().pays_fee, Pays::No);

		// acc 3 modify the vote, expecting Ok with Pays::Yes.
		let vote_rval: DispatchResultWithPostInfo = Collective::vote(Origin::signed(3), hash, 0, false);
		assert_eq!(vote_rval.unwrap().pays_fee, Pays::Yes);
...
fn migration_v4() {
	new_test_ext().execute_with(|| {
		...
		frame_support::storage::migration::move_pallet(new_pallet.as_bytes(), old_pallet.as_bytes());
